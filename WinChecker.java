...
public class WinChecker {
	...
	// this is a separate object so that new versions can quickly and easily be
	// made
	NegatedGoal goal;
	...
	/**
	 * Checks for a win, but only for the player whose KB is passed in as an
	 * argument
	 */
	private void resolve(LinkedList<EdgeAxiom> KB) {
		...
		while (counter < KB.size()) {
			// starts with a new negated goal, since those from previous loops
			// will be modified
			goal = new NegatedGoal();
			...
			for (EdgeAxiom e : KB) {
				...
				// returns a win if the unification makes the goal an empty
				// clause
			...
			// increments counter
			counter++;
			// takes the first axiom in the knowledge base and moves it to the
			// end
			rotateKB(KB);
		}
...
//		}
	...
	public boolean getWinForPlayerX() {
		return winForPlayer1;
	}

	public boolean getWinForPlayerO() {
